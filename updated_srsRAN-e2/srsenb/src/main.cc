...
#include "lib/include/srsran/interfaces/enb_metrics_interface.h"
...
struct training_metrics_t {
  ...
  //std::vector <int> cqi_rbg; // store CQI for each RBG
  //std::vector <float> dl_datarate; // downlink bit rate for each ue
  int cqi_rbg[10];
  float dl_datarate[10];
};
void wait_ms(int P){
  ...
  do {
    ...
    if (elapsed.count() == P )condition=true;}
  while (condition==false);
}
/*
training_metrics_t training_info_collect(enb_metrics_t metrics, sf_sched tti_sched, sched_dl_cqi dl_cqi)
{
...
int main(int argc, char* argv[])
{
  ...
  struct enb_metrics_t               enb_metrics;
  sched_dl_cqi                       enb_dl_cqi(50,1,5);
  int training_rlt[21];
  //sched_dl_cqi                       enb_dl_cqi;
  //sf_grid_t                           enb_tti_sched;
  cout << "---  Software Radio Systems LTE eNodeB  ---" << endl << endl;

////////////This sections is for socket/////////////////////////////////////
////////////////////////////////////////////////////////////////////////////
  int sock = 0, rcv_sock=0, valread, client_fd;
  struct training_metrics_t training_info;

  struct sockaddr_in serv_addr;
  std::vector <uint32_t> rcvstruct;
  std::vector<uint32_t>training_output;
  cout << "...  Initialize Socket Client..., wait for connection..." << endl;
  ...
  cout << "Cliend has been connected" << endl;
///////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////
  ...
  /*
     while (running){
       std::cout<<"socket tx tx... "<<std::endl;
       wait_ms(1000);
     }
     */

    //uint32_t alloc_rlt[]={2, 4, 6, 8, 3, 5, 7, 9};
    //const sched_dl_cqi& dl_cqi() const { return enb_dl_cqi; }
    //std::ofstream myfile;

    std::vector<int>tmp_cqi_rbg;
    std::vector<float>tmp_dl_datarate;
    training_info.nof_ues=0;
    for (uint32_t mn=0; mn<10; ++mn) {
      training_info.dl_datarate[mn]=0;
      training_info.cqi_rbg[mn]=0;
      //myfile<<"data rate = "<<tmp_dl_datarate[mn];
    }


    while (running) {
      //training_info = metrics_screen.training_info_collecttraining_info_collect(const enb_metrics_t& metrics);
      //if (enb->init(args) == SRSRAN_SUCCESS) {
      //enb->get_metrics(&enb_metrics);
      /*
      if (training_info.dl_datarate.size()>0)
          training_info.dl_datarate.clear();
      if (training_info.cqi_rbg.size()>0)
          training_info.cqi_rbg.clear();
      */
      if (tmp_dl_datarate.size()>0)
          tmp_dl_datarate.clear();
      if (tmp_cqi_rbg.size()>0)
          tmp_cqi_rbg.clear();
      std::tuple<int32_t, std::vector<int>, std::vector<float>>rlt= metricshub.enb_metrics_output();
      training_info.nof_ues=std::get<0>(rlt);
      tmp_cqi_rbg=std::get<1>(rlt);
      tmp_dl_datarate=std::get<2>(rlt);

      //training_info.cqi_rbg=std::get<1>(rlt);
      //training_info.dl_datarate=std::get<2>(rlt);
      //myfile<<training_info.nof_ues;
      for (uint32_t mn=0; mn<training_info.nof_ues; ++mn) {
        training_info.dl_datarate[mn]=tmp_dl_datarate[mn];
        training_info.cqi_rbg[mn]=tmp_cqi_rbg[mn];
        //myfile<<"data rate = "<<tmp_dl_datarate[mn];
      }


      std::cout<<"number of active ues = "<<training_info.nof_ues<<std::endl;
      std::cout<<"data rate = ";
      for (uint32_t ue=0; ue<training_info.nof_ues; ++ue) {
        std::cout<<training_info.dl_datarate[ue]/1000<<" kbps"<< ", CQI = "<<training_info.cqi_rbg[ue]<<std::endl;
        //std::cout<<training_info.dl_datarate[ue]<<", "<<training_info.cqi_rbg[ue] <<std::endl;
      }
      std::cout<<std::endl;

      send(sock, (const void *)&training_info, sizeof(struct training_metrics_t), 0);
      //send(sock, alloc_rlt, sizeof(alloc_rlt), 0);
      memset(training_rlt, 0, sizeof(training_rlt));
      wait_ms(10);
      if (recv(rcv_sock, training_rlt, 21, 0) > 0 ) {
        //valread = read(rcv_sock, training_rlt, 21);
        std::cout<<" training results = ";
        for (uint32_t idx=0; idx<21; idx++ ) {
          std::cout<<training_rlt[idx]<<" ";
        }
        std::cout<<std::endl;
        break;
      }
      ...
      //training_info.dl_mask=enb_tti_sched.get_dl_mask();
      //std::cout<<" dl mask size ="<<training_info.dl_mask.size()<<std::endl;

      //const uint32_t mask_size=training_info.dl_mask.size();
      /*
      std::bitset<25> my_mask;
      for (uint32_t id=0;id<25;++id) {
        if (training_info.dl_mask.test(id))
          my_mask.reset(id);
        else
          my_mask.set(id);
      }
      */
      ...
      /*
      for (uint32_t sb_idx=0;sb_idx<max_nof_subbands;++sb_idx) {
          training_info.cqi_rbg.push_back(enb_dl_cqi.get_subband_cqi(sb_idx));
          std::cout<<"  cqi = "<<enb_dl_cqi.get_subband_cqi(sb_idx);
      }

      if (training_info.dl_datarate.size()>0)
          training_info.dl_datarate.clear();

      for (uint32_t ue_idx=0;ue_idx<enb_metrics.stack.rrc.ues.size();++ue_idx) {
        float signle_ue_rate = enb_metrics.stack.mac.ues[ue_idx].tx_brate / (enb_metrics.stack.mac.ues[ue_idx].nof_tti * 1e-3);
        training_info.dl_datarate.push_back(signle_ue_rate);
      } */
       //std::cout<<"in main 1: active ues = "<<enb_metrics.stack.rrc.ues.size()<<std::endl;
  //    std::cout<<"dl mask = "<<training_info.dl_mask<<std::endl;
      //std::cout<<"CQI = "<<training_info.cqi_rbg[0]<<std::endl;
      //std::cout<<"data rate = "<<training_info.dl_datarate[0]<<std::endl;
      //training_info = training_info_collect(enb_metrics, enb_tti_sched, enb_dl_cqi);

      //wait_ms(2000);

      //valread = read(sock, (void*) &rcvstruct, sizeof(rcvstruct));

    }

  //std::cout<<"in main 2: active ues = "<<enb_metrics.stack.rrc.ues.size()<<std::endl;
  int cnt    = 0;
  int ts_cnt = 0;
  while (running) {
