...
public:
  ...
  virtual void stop()                                                      = 0;
  virtual std::tuple<int32_t, std::vector<int>, std::vector<float>> cal_metrics(const metrics_t& m) = 0;
...
public:
  ...
  std::tuple<int32_t, std::vector<int>, std::vector<float>> enb_metrics_output()
  {
    int32_t nof_ues=0;
    std::vector<float> dr;
    std::vector<int> cqi;

    std::tuple<int32_t, std::vector<int>, std::vector<float>>rlt;

     if (m) {
      metrics_t metric = {};
      m->get_metrics(&metric);
      for (uint32_t i = 0; i < listeners.size(); i++) {
        rlt=listeners[i]->cal_metrics(metric);
        nof_ues+=(std::get<0>(rlt));
        cqi.insert(cqi.end(),(std::get<1>(rlt)).begin(),(std::get<1>(rlt)).end());
        dr.insert(dr.end(),(std::get<2>(rlt)).begin(),(std::get<2>(rlt)).end());
      }
    }
    return std::make_tuple(nof_ues,cqi,dr);
  }
